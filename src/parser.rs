// use crate::{ast::EExpr, ast::ELhs, ast::Spanned, lexer::Token};
// use chumsky::prelude::*;

// pub fn expr_parser() -> impl Parser<Token, Spanned<EExpr>, Error = Simple<Token>> {
//     let expr = recursive(|expr: Recursive<Token, Spanned<EExpr>, Simple<Token>>| {
//         let val = select! {
//             Token::Unit => EExpr::Unit,
//             Token::Num(n) =>  EExpr::Num(n.parse().unwrap()),
//         }
//         .map_with_span(|e, s| (e, s))
//         .labelled("value");

//         let lhs = recursive(|lhs: Recursive<Token, Spanned<ELhs>, Simple<Token>>| {
...
//                 .ignore_then(oppar)
//                 .map_with_span(|lhs, s| ELhs::DeRef(Box::new(lhs), s));

//             // .to(ELhs::DeRef as fn(_, _) -> _)
//             // .repeated()
//             // .then(oppar)
...
//         let atom = lhs
//             .clone()
//             .map_with_span(|e, s| (EExpr::Lvalue(e), s))
//             .or(val);
...
//             .ignore_then(oppar)
//             .map_with_span(|e, s| (EExpr::Neg(Box::new(e)), s));
